...
/*
 * 
 * @类描述：将大集群分割，然后多线程运行各个集群的choco约束编程。THRESHOLD决定分割物理机的阈值。物理机、虚拟机资源的生成都是随机的。顺序分割物理机，
...
public static void main(String[] agrs){
	...
	try{
		...
		ChocoProcess cp[] = new ChocoProcess[groupNum];
		/*
...
//				segmentVMN = VMN - vmGap*i;
			...
			cp[i] = new ChocoProcess(segmentPMN,segmentVMN,segvmCPU,segvmRAM,segpmCPU,segpmRAM,segmentstaticPow,segmentDYP);	
		}
		
		Thread[] th = new Thread[groupNum];
		for(int j = 0; j < groupNum; j++){
			th[j] = new Thread(cp[j]);
		}
